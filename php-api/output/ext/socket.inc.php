...
class c_socket
{
    ...
    function tcp_connect($address, $port, $ctimeout = NULL)
    {
        ...
        $r = array($socket);
        $w = array($socket);
        $ret = @ socket_select($r, $w, $e = NULL, $ctimeout, 0); //尝试屏蔽错误
    ...
    function is_connected()
    {
        if ($this->socket !== false || $this->socket !== NULL)
        {
            if(is_resource($this->socket))
            {
                if (socket_getpeername($this->socket, $address))
                {
                    return true;
                }
                else
            ...
            } // end of socket is a resource
        }
        return false;
    }
    
    // 增加三个返回值，反回readable状态
    const ERROR   = -2;
    const TIMEOUT = -1;
    const OK      =  0;
    /**
     * 测试socket是否可读
     * @param number $timeo:读超时
     */
    function is_readable($timeo_s, $timeo_us)
    {
        $read = array($this->socket);
        $write = null;
        $except = null;
        $num_readable_sockets = @socket_select($read, $write, $except, $timeo_s, $timeo_us);
        
        if (FALSE === $num_readable_sockets)
        {
            $this->socket_errno = socket_last_error();
            $this->socket_error = socket_strerror($this->socket_errno);
            return c_socket::ERROR; // ERROR
        }
        else if (0 === $num_readable_sockets) 
        {
            return c_socket::TIMEOUT; // time out
        } 
        else
        {
            return c_socket::OK; // ok
        }
    }
}
?>
